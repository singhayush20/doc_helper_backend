...
import com.ayushsingh.doc_helper.features.user_activity.repository.UserActivityUpsertRepository;
...
public class UserActivityBatchFlusher {
    ...
    private final UserActivityUpsertRepository upsertRepository;
    ...
    void doFlush() {
        ...
        List<UserActivityWriteRequest> requests =
                entries.values().stream()
                        .map(v -> deserialize(v.toString()))
                        .toList();

        upsertRepository.batchUpsert(requests);

        redis.delete(BUFFER_KEY);

        log.debug("Flushed {} user activities", requests.size());
    }

    private UserActivityWriteRequest deserialize(String json) {
        try {
            return objectMapper.readValue(json, UserActivityWriteRequest.class);
        } catch (Exception e) {
